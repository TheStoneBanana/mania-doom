...
{
    V_DrawPatch(SCREENDELTA_W, 0, background);
}
  ...
  patch_t*	c[] )
{
    ...
    do
    {
	left = (int)((float)(lnodes[wbs->epsd][n].x - SHORT(c[i]->leftoffset)) + (float)SCREENDELTA_W);
	top = (int)((float)(lnodes[wbs->epsd][n].y - SHORT(c[i]->topoffset)) * ((float)SCREENHEIGHT / (float)SCREENHEIGHT));
    ...
    if (fits && i<2)
    {
	V_DrawPatch(lnodes[wbs->epsd][n].x + SCREENDELTA_W,
                    (lnodes[wbs->epsd][n].y * ((float)SCREENHEIGHT / (float)SCREENHEIGHT)),
		    c[i]);
    }
    else
    {
...
void WI_drawAnimatedBack(void)
{
	...
	if (a->ctr >= 0)
	    V_DrawPatch(a->loc.x + SCREENDELTA_W, a->loc.y, a->p[a->ctr]);
    }
...
void WI_drawStats(void)
{
	...
	V_DrawPatch(SP_STATSX + SCREENDELTA_W, SP_STATSY, kills);
	WI_drawPercent((SCREENORIG_W - SP_STATSX) + SCREENDELTA_W, SP_STATSY, cnt_kills[0]);

	V_DrawPatch(SP_STATSX + SCREENDELTA_W, SP_STATSY + lh, items);
	WI_drawPercent((SCREENORIG_W - SP_STATSX) + SCREENDELTA_W, SP_STATSY + lh, cnt_items[0]);

	V_DrawPatch(SP_STATSX + SCREENDELTA_W, SP_STATSY + 2 * lh, sp_secret);
	WI_drawPercent((SCREENORIG_W - SP_STATSX) + SCREENDELTA_W, SP_STATSY + 2 * lh, cnt_secret[0]);

	V_DrawPatch(SP_TIMEX + SCREENDELTA_W, SP_TIMEY, timepatch);
	WI_drawTime((SCREENORIG_W / 2 - SP_TIMEX) + SCREENDELTA_W, SP_TIMEY, cnt_time);

	if (wbs->epsd < 3)
	{
		V_DrawPatch((SCREENORIG_W / 2 + SP_TIMEX) + SCREENDELTA_W, SP_TIMEY, par);
		WI_drawTime((SCREENORIG_W - SP_TIMEX) + SCREENDELTA_W, SP_TIMEY, cnt_par);
	}
